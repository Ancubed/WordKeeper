...
const sortFunction = (
    selector: FetchOneWordDataResultInterface[],
    filter: PartOfSpeechType | null,
    text: string,
    ...
    if (
        sortType === 'alphabet' ||
        (filter === null && text === '' && sortType !== 'custom')
    ) {
...
const Favourites: React.FC<DefaultPropsInterface> = ({
    className,
    ...
    let wordsSearchData = useSelector((state: StoreState) =>
        sortFunction(state.search.words, filter, text, typeOfSort, false)
    )

    const localSearchCallback = (
        newFilter: PartOfSpeechType | null,
        newText: string
    ) => {
    ...
    return (
        <div className={classname('', className)}>
            ...
            <div className="mt-4">
                ...
                />
                <FavouritesWordsList
                    wordsSearchData={wordsSearchData}
                    dragAndDropEnabled={typeOfSort == 'custom'}
                />
            </div>
        </div>
    )
