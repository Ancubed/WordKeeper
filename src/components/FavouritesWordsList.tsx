...
const FavouritesWordsList: React.FC<FavouritesWordListPropsInterface> = ({ className, wordsSearchData, dragAndDropEnabled }: FavouritesWordListPropsInterface) => {
  const dispatch = useDispatch();
  
  const onDragEnd = (result: any) => {
    if (!result.destination) return;
    dispatch(reorder({ source: result.source, destination: result.destination }))
  }
  ...
  if (dragAndDropEnabled) return (
      <DragDropContext onDragEnd={onDragEnd}>
        <Droppable droppableId="words">
          {(provided) => (
            <div className={classname("select-none mt-4", className)} {...provided.droppableProps} ref={provided.innerRef}>
                {wordsSearchData.map((wrd, index) => 
                  <Draggable key={wrd.word} draggableId={wrd.word} index={index}>
                    {(provided) => (
                      <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                        <Word word={wrd.word} propsWordSearchData={wrd} isDraggable={dragAndDropEnabled}/>
                      </div>
                    )}
                  </Draggable>
                )}
